...
1. Have a GPG key whose public half you will register. If you do not have one:
   `gpg --full-generate-key`, then export the public key
   (`gpg --armor --export <fingerprint>`).
2. Sign in to [registry.terraform.io](https://registry.terraform.io) with the
   TAIPANBOX GitHub account, and under the namespace's settings add that
   **public** GPG key and connect this repository as a provider.
3. Add two repository secrets (Settings -> Secrets and variables -> Actions):
   `GPG_PRIVATE_KEY` (the ASCII-armored private key) and `PASSPHRASE` (its
   passphrase, or leave empty if none). The private key never leaves the repo
   secret store.

**Cutting a release:**

- With Actions (once the account's Actions are active): push a version tag and
  the `release` workflow builds, signs, and publishes it.
  ```sh
  git tag v0.1.0
  git push origin v0.1.0
  ```
- Without Actions (run it locally, needs the private GPG key in your keyring):
  ```sh
