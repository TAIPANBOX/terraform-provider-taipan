...
GPG-signed, at 0.1.0 since 23 July 2026. See "Publishing a release" below for
how a release is actually cut, which is not yet the tag-triggered path. Add it
to a `required_providers` block and run `terraform init`, same as any other
provider:
...
`.goreleaser.yml` cross-compiles every platform, writes the `SHA256SUMS`
checksum plus `terraform-registry-manifest.json`, and detach-signs the
checksums; the Registry verifies that signature against a public key you
register. `docs/` (generated by `tfplugindocs`) is what the Registry renders as
the provider documentation.

**One-time setup** (needs a person with the TAIPANBOX GitHub org and a GPG key).
Two of the three are done:

1. **Done.** A GPG key exists and its public half was exported for registration
   (`gpg --full-generate-key`, then `gpg --armor --export <fingerprint>`).
2. **Done.** That public key is registered under the TAIPANBOX namespace on
   [registry.terraform.io](https://registry.terraform.io) and this repository is
   connected as a provider, which is why `TAIPANBOX/taipan` 0.1.0 resolves.
3. **Not done.** Two repository secrets (Settings -> Secrets and variables ->
   Actions): `GPG_PRIVATE_KEY` (the ASCII-armored private key) and `PASSPHRASE`
   (its passphrase, or leave empty if none). The private key never leaves the
   repo secret store. Until these exist the tag-triggered path cannot sign
   anything, so it is not the path to reach for.

**Cutting a release.** Only the local path has ever produced a release, and
0.1.0 went out that way:

- **Locally**, needs the private GPG key in your keyring:
  ```sh
  export GPG_FINGERPRINT=<your key fingerprint>
  export GITHUB_TOKEN=<a token with repo scope>
  goreleaser release --clean
  ```
- **By tag**, once step 3 above is done: push a version tag and the `release`
  workflow builds, signs and publishes it.
  ```sh
  git tag v0.1.1
  git push origin v0.1.1
  ```
  It has run exactly once, on the v0.1.0 tag, and failed after five seconds with
  `Input required and not supplied: gpg_private_key`. The release carrying
  0.1.0's thirteen signed archives was made locally afterwards. Actions
  themselves are active and `ci` is green on every push, so what is missing is
  the secret, not the runner.

Either way, once the signed release exists on a connected repository the
Registry ingests it within a few minutes, and `source = "TAIPANBOX/taipan"`
resolves with a plain `terraform init`. Regenerate `docs/` before a release if
resource schemas changed: `tfplugindocs generate --provider-name taipan`.
